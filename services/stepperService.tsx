...
import type { 
  ...
  Fan,
  AC
} from "@/store/useStepperStore";
import type { AppRouterInstance } from "@/types/next-router";
...
export const fetchStepperData = async (
  ...
  stepperType: string,
  productId: string,
  router: AppRouterInstance // FIX: Changed from NextRouter to AppRouterInstance
): Promise<StepperResponse> => {
  try {
    const token = localStorage.getItem("NECCA_AUTH_TOKEN") || "";
